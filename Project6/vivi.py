...
def main(foilfile, alfa, Niter=0, Vinf=1, mu=1.79E-5, rho=1.225):
    """Run VIvI for specified number of iterations.
    foilfile --> path to airfoil geometry file
    alfa --> Single AoA at which to perform VIvI
    Niter --> total number of VIvI iterations to run
    Vinf, mu, rho --> freestream conditions
    """

    soln = [] #store solution for each iteration
    for niter in range(Niter+1):
        print(niter)

        #RUN VISCOUS/INVISCID INTERACTION
            #returns dicitonary with dataframes for upper/lower surface
                #"displaced" geometry
                #momentum thickness, Surface pressure, surface velocity
        dfs = VIvI(foilfile, alfa, niter=niter, Vinf=Vinf, mu=mu, rho=rho)

        #STORE CURRENT SOLUTION
        soln.append(dfs.copy())
...
if __name__ == "__main__":
    ...
    # #Very cambered
    #     #FLAT PLATE ASSUMPTION FAILS
    #     #code doesnt work well for this, too much visc decambering
    #     #lift/drag estimate gets worse with each it
    # for i in range(3):
    #     print(i)
    #     main('s1223', 0, i)
    ...
    alpha = 0
    Name = 'naca2412'
    soln = main(Name, alpha, N)
    # Analysis(soln, Name, alpha)
